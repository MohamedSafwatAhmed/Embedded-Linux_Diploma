...
with basic features contains \n
    ...
    4) ACC state indicator. \n
    5) LKA state indicator. \n
    6) LKW state indicator. \n
    7) Accelerator state indicator. \n
    8) Break state indicator. \n
    9) Horn state indicator. \n
    10) Left and right indicator. \n
    10) You can control the above states by using several function provided in 'TriggerAction' class.\n\n
Some cool features: \n
...
class _DashBoardMain(QWidget):
    ...
    def customKeyPressEvent(self, event):
        ...
        if event.key() == Qt.Key.Key_Space or self.keys_[Qt.Key.Key_Space]:
            self.keys_[event.key()] = True
        #Acc
        if event.key() == Qt.Key.Key_P or self.keys_[Qt.Key.Key_P]:
            self.keys_[event.key()] = True
        #LKW
        if event.key() == Qt.Key.Key_O or self.keys_[Qt.Key.Key_O]:
            self.keys_[event.key()] = True
        #LKA
        if event.key() == Qt.Key.Key_I or self.keys_[Qt.Key.Key_I]:
            self.keys_[event.key()] = True
    ...
    def customKeyReleaseEvent(self, event):
        ...
        if event.key() == Qt.Key.Key_P and not event.isAutoRepeat():
            self.keys_[event.key()] = False
            self.dash_board_design_widget.set_ACC_state(0)
        #LKW
        if event.key() == Qt.Key.Key_O and not event.isAutoRepeat():
            self.keys_[event.key()] = False
            self.dash_board_design_widget.set_LKW_state(0)
        
        #LKA
        if event.key() == Qt.Key.Key_I and not event.isAutoRepeat():
            self.keys_[event.key()] = False
            self.dash_board_design_widget.set_LKA_state(0)
    ...
    def keyAction(self):
        if any(self.keys_.values()):
            ...
            if self.keys_[Qt.Key.Key_P]:
                self.dash_board_design_widget.set_ACC_state(1)
            if self.keys_[Qt.Key.Key_P]:
                self.dash_board_design_widget.set_ACC_state(1)
            
            #LKW
            if self.keys_[Qt.Key.Key_O]:
                self.dash_board_design_widget.set_LKW_state(1)
            if self.keys_[Qt.Key.Key_O]:
                self.dash_board_design_widget.set_LKW_state(1)
            
             #LKA
            if self.keys_[Qt.Key.Key_I]:
                self.dash_board_design_widget.set_LKA_state(1)
            if self.keys_[Qt.Key.Key_I]:
                self.dash_board_design_widget.set_LKA_state(1)
...
class _DashBoardContolsDesign(QWidget):
    ...
    def ACC_properties(self):
        self.ACC_default_state = 0
        self.ACC_state = 0
        self.ACC_color_lst = (QColor(67, 13, 13, 200), QGradient(QGradient.Preset.HeavyRain))
    
    def LKW_properties(self):
        self.LKW_default_state = 0
        self.LKW_state = 0
        self.LKW_color_lst = (QColor(67, 13, 13, 200), QGradient(QGradient.Preset.HeavyRain))
    
    def LKA_properties(self):
        self.LKA_default_state = 0
        self.LKA_state = 0
        self.LKA_color_lst = (QColor(67, 13, 13, 200), QGradient(QGradient.Preset.HeavyRain))


    def set_ACC_state(self, val):
        self.ACC_default_state = val
        self.ACC_state = val
        self.repaint()

    def set_LKA_state(self, val):
        self.LKA_default_state = val
        self.LKA_state = val
        self.repaint()
    
    def set_LKW_state(self, val):
        self.LKW_default_state = val
        self.LKW_state = val
        self.repaint()

    def ACC_painting(self, painter: QPainter):
        # setting ACC font
        ACC_font = QFont("Consolas", 0, 0, True)
        ACC_font.setPixelSize(round(self.width()*0.050))
        ACC_fm = QFontMetrics(ACC_font)
        ACC_rect = ACC_fm.boundingRect("ACC") ##ACC
        painter.setFont(ACC_font)

        ACC_rect.moveTo(self.rect().center()+QPointF(self.rect().width()*0.3, -self.rect().height()*0.2).toPoint())

        painter.setPen(QPen(self.ACC_color_lst[self.ACC_state], round(self.width()*0.0025)))
        painter.drawText(ACC_rect, Qt.AlignmentFlag.AlignCenter, "ACC")
     #new part add here
    def LKA_painting(self, painter: QPainter):
        # setting LKA font
        LKA_font = QFont("Consolas", 0, 0, True)
        LKA_font.setPixelSize(round(self.width()*0.050))
        LKA_fm = QFontMetrics(LKA_font)
        LKA_rect = LKA_fm.boundingRect("LKA") ##ACC
        painter.setFont(LKA_font)

        LKA_rect.moveTo(self.rect().center()+QPointF(-self.rect().width()*0.4, -self.rect().height()*0.2).toPoint())

        painter.setPen(QPen(self.LKA_color_lst[self.LKA_state], round(self.width()*0.0025)))
        painter.drawText(LKA_rect, Qt.AlignmentFlag.AlignCenter, "LKA")
    
    def LKW_painting(self, painter: QPainter):
        # setting LKW font
        LKW_font = QFont("Consolas", 0, 0, True)
        LKW_font.setPixelSize(round(self.width()*0.050))
        LKW_fm = QFontMetrics(LKW_font)
        LKW_rect = LKW_fm.boundingRect("LKW") ##AKW
        painter.setFont(LKW_font)

        LKW_rect.moveTo(self.rect().center()+QPointF(-self.rect().width()*0.4, -self.rect().height()*0.008).toPoint())

        painter.setPen(QPen(self.LKW_color_lst[self.LKW_state], round(self.width()*0.0025)))
        painter.drawText(LKW_rect, Qt.AlignmentFlag.AlignCenter, "LKW")
    ...
    def break_painting(self, painter: QPainter):
        ...
        break_font.setPixelSize(round(self.width()*0.050))
        break_fm = QFontMetrics(break_font)
        break_rect = break_fm.boundingRect("AEB")  #name change
        painter.setFont(break_font)

        break_rect.moveTo(self.rect().center()+QPointF(self.rect().width()*0.3,  -self.rect().height()*0.008).toPoint())

        painter.setPen(QPen(self.break_color_lst[self.break_state], round(self.width()*0.0025)))
        painter.drawText(break_rect, Qt.AlignmentFlag.AlignCenter, "AEB") #Changed
    ...
    def set_accelerator_state(self, val):
        ...
        if self.speed <= 300 and not self.break_state:
            self.speed += 4*self.speed_angle_factor
            self.repaint()
    
    def Driving_painting(self, painter: QPainter):
        # setting accelerator font
        accelerator_font = QFont("Consolas", 0, 0, True)
        accelerator_font.setPixelSize(round(self.width()*0.050))
        accelerator_fm = QFontMetrics(accelerator_font)
        accelerator_rect = accelerator_fm.boundingRect("Driving")
        painter.setFont(accelerator_font)

        accelerator_rect.moveTo(self.rect().center()+QPointF(-self.rect().width()*.110, self.rect().height()*0.320).toPoint())

        painter.setPen(QPen(self.accelerator_color_lst[self.accelerator_state], round(self.width()*0.0025)))
        painter.drawText(accelerator_rect, Qt.AlignmentFlag.AlignCenter, "Driving")

    def speedometer_properties(self):
        self.speedometer_bounding_rect = QRectF(self.width()*0.300, self.height()*1.01, self.width()*0.4, self.width()*0.4)
        ...
        if self.speed>0 and self.enable_speedometer_resetter:
            if not self.accelerator_state: # accelerator released
                if self.break_state: # break presssed
                    self.speed -= 7*self.speed_angle_factor
                else: 
    ...
    def paintEvent(self, event):
        ...
        if self.other_visible:
            ...
            self.ACC_painting(painter)
            self.LKA_painting(painter)
            self.LKW_painting(painter)
            self.break_painting(painter)
            self.Driving_painting(painter)
...
class _DashBoardControls(QObject):
    ...
    ACC_sig = pyqtSignal(int)
    LKA_sig = pyqtSignal(int)
    LKW_sig = pyqtSignal(int)
    ...
    def required_values(self):
        ...
        self.ACC_state = 0 # off
        self.LKA_state = 0 # off
        self.LKW_state = 0 # off
    ...
    def startup_values_setter(self):
        ...
        self.dash_board.dash_board_design_widget.set_ACC_state(self.ACC_state)
        self.dash_board.dash_board_design_widget.set_LKA_state(self.LKA_state)
        self.dash_board.dash_board_design_widget.set_LKW_state(self.LKW_state)
    ...
    def all_connector(self):
        ...
        self.ACC_sig.connect(self.dash_board.dash_board_design_widget.set_ACC_state)
        self.LKW_sig.connect(self.dash_board.dash_board_design_widget.set_LKW_state)
        self.LKA_sig.connect(self.dash_board.dash_board_design_widget.set_LKA_state)
...
class TriggerAction():
    ...
    def ACC_on(self):
        """To indicate ACC is on"""
        self.__dbc.ACC_on()

    def ACC_off(self):
        """To indicate ACC is off"""
        self.__dbc.ACC_off()
    
    def LKA_on(self):
        """To indicate LKA is on"""
        self.__dbc.LKA_on()

    def LKA_off(self):
        """To indicate LKA is off"""
        self.__dbc.LKA_off()

    def LKW_on(self):
        """To indicate LKW is on"""
        self.__dbc.LKW_on()

    def LKA_off(self):
        """To indicate LKW is off"""
        self.__dbc.LKW_off()
